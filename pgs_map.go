...
if len(os.Args)<2   {log.Printf("Run it:\n%v -code script_file.gomacro [-mask \"%v\"] [-tracing] < input_file\n",os.Args[0])
   ...
   if *tracing  {log.Println("Code:\n",codeContent)}
    ...
    for scanner.Scan() {
        fmt.Println(RunGomacro2(*tracing,codeContent,scanner.Text())) // Println will add back the final '\n'
    }
...
func RunGomacro2(tracing bool,code string,toeval string) string {
    interp := fast.New()
